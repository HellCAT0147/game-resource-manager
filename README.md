...
- **Без спойлеров**: стартового каталога нет — все найденные предметы добавляешь вручную (название, цена в крб, вес в кг, количество, категория).
- **Выгодность (крб/кг)** у каждого предмета на видном месте; цветовая метка показывает выгодное / среднее / «копейки».
- **Группы по категориям**: еда, пистолет, автомат, дробовик, патроны, броня, другое, артефакт, граната, медикаменты.
...
| `stalker.js` | логика S.T.A.L.K.E.R. 2: выгодность крб/кг, оптимизатор «что нести», категории, импорт/экспорт |
